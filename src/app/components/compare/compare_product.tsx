...
export default function Compare() {
  ...
  const fetchData = async () => {
    try {
      const productResponse = await dispatch(
        findProduct(select_compareproduct.product_compare)
      );
      ...
      const typesPromises = products.map(async (row: any) => {
        ...
        return find_typesProduct(
          row.attributes.product_brand.data?.attributes.brand_name
        );
      });
  ...
  const remove_compareProduct = async (id: number) => {
    const updatedArray = select_compareproduct.product_compare.filter(
          ...
          >
            ...
            >
              ...
              <tbody>
                <tr>
                  <td
                    style={{ backgroundColor: "#f5f5f5", border: "0px" }}
                  ></td>
                ...
                <tr>
                  <td>
                    <ThemeProvider theme={font}>
                      <Typography sx={{ fontSize: "16px" }}>TYPES</Typography>
                    </ThemeProvider>
                  </td>
                  <td>
                    <ThemeProvider theme={font}>
              ...
              >
                ...
                >
                      ...
                      >
                        <Box sx={{ justifyContent: "center", display: "flex" }}>
                          <Button
                            sx={{
                              color: "#bb3c23",
                              border: "1px solid #bb3c23",
                              transition: "0.2s",
                              ":hover": { bgcolor: "#bb3c23", color: "white" },
                            }}
                            onClick={handleClearCompareList}
                          >
                            <ThemeProvider theme={font}>
                              <Typography
                                sx={{ fontSize: isDesktop ? "15px" : "13px" }}
                              >
                                Clear compare list
                              </Typography>
                            </ThemeProvider>
                      ...
                      {productData.map((row: any, index: any) => (
                        ...
                        >
                          <Grid item xs={12}>
                            ...
                            <ThemeProvider theme={font}>
                              <Typography
                                sx={{ color: "#ee4d2d", textAlign: "right" }}
                              >
                                <div
                                  onClick={() => remove_compareProduct(row.id)}
                                >
                                  <HighlightOffIcon
                                    sx={{
                                      fontSize: "25px",
                  ...
                  <tbody>
                    ...
                    {productData.some(
                      (row: any) =>
                        row.attributes.product_width.data?.attributes.width_name
                    ) && (
                      <tr>
                        <td>
                          <ThemeProvider theme={font}>
                    ...
                    {productData.some(
                      (row: any) =>
                        row.attributes.product_ratio.data?.attributes.ratio_name
                    ) && (
                      <tr>
                        <td>
                          <ThemeProvider theme={font}>
                        ...
                        {productData.map((row: any, index: any) => (
                          <td key={index}>
                            <ThemeProvider theme={font}>
                              <Typography sx={{ fontSize: "15px" }}>
                                {
                                  row.attributes.product_diameter.data
                    ...
                    {productData.some(
                      (row: any) =>
                        row.attributes.product_typesofengineoil.data?.attributes

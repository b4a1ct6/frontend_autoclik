...
import Allproduct_filter from "./allproduct_filter";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const storedTireDiameter: any = localStorage.getItem(
          "checkedTireDiameter"
        );
        ...
        const storedLubeOilBrand: any = localStorage.getItem(
          "checkedLubeOil_Brand"
        );
        const storedLubeOilBrand_json = JSON.parse(storedLubeOilBrand);
        const storedLubeOilFluidTypes: any = localStorage.getItem(
          "checkedLubeOil_Fluid_type"
        );
        const storedLubeOilFluidTypes_json = JSON.parse(
          storedLubeOilFluidTypes
        );
        const storedLubeOil_engineOil_Typesofengineoil: any =
          localStorage.getItem("checkedLubeOil_engineOil_Typesofengineoil");
        const storedLubeOil_engineOil_Typesofengineoil_json = JSON.parse(
          storedLubeOil_engineOil_Typesofengineoil
        );

        const storedShockAbsorberBrands: any = localStorage.getItem(
          "checkedShockAbsorberBrands"
        );
        const storedShockAbsorberBrands_json = JSON.parse(
          storedShockAbsorberBrands
        );
        const storedShockAbsorberShockuptype: any = localStorage.getItem(
          "checkedShockAbsorberShockuptype"
        );
        const storedShockAbsorberShockuptype_json = JSON.parse(
          storedShockAbsorberShockuptype
        );

        const storedBrakeBrands: any =
          localStorage.getItem("checkedBrakeBrands");
        const storedBrakeBrands_json = JSON.parse(storedBrakeBrands);
        const storedBrakeBraketype: any = localStorage.getItem(
          "checkedBrakeBraketype"
        );
        const storedBrakeBraketype_json = JSON.parse(storedBrakeBraketype);

        const storedBatteryBrands: any = localStorage.getItem(
        ...
        const storedOtherproductBrands: any = localStorage.getItem(
          "checkedOtherproductBrands"
        );
        const storedOtherproductBrands_json = JSON.parse(
          storedOtherproductBrands
        );
        ...
        const tireData =
          products.product_filter_results.product_filter_results?.tire || [];
        const lubeOilData =
          products.product_filter_results.product_filter_results?.lube_oil ||
          [];
        const shockAbsorberData =
          products.product_filter_results.product_filter_results
            ?.shock_absorber || [];
        const brakeData =
          products.product_filter_results.product_filter_results?.brake || [];
        const batteryData =
          products.product_filter_results.product_filter_results?.battery || [];
        ...
        if (storedTypes_json[0] === "other_product") {
          const filteredData = otherproductData.filter((item: any) => {
            ...
            const brandMatch =
              brand_name.length === 0 ||
              brand_name.includes(
                item.attributes.product_brand.data.attributes.brand_name
              );
        ...
        if (storedTypes_json[0] === "battery") {
          const filteredData = batteryData.filter((item: any) => {
            ...
            const ampMatch =
              amp_name.length === 0 ||
              amp_name.includes(
                item.attributes.product_amp.data.attributes.amp_name
              );
        ...
        if (storedTypes_json[0] === "brake") {
          const filteredData = brakeData.filter((item: any) => {
            ...
            const braketypeMatch =
              braketype_name.length === 0 ||
              braketype_name.includes(
                item.attributes.product_braketype.data.attributes.braketype_name
              );
        ...
        if (storedTypes_json[0] === "shock_absorber") {
          const filteredData = shockAbsorberData.filter((item: any) => {
            const { brand_name, shockuptype_name } =
              combinedShockAbsorberFilterData;
            ...
            const shockuptypeMatch =
              shockuptype_name.length === 0 ||
              shockuptype_name.includes(
                item.attributes.product_shockuptype.data.attributes
                  .shockuptype_name
              );
        ...
        if (storedTypes_json[0] === "lube_oil") {
          const filteredData = lubeOilData.filter((item: any) => {
            const { brand_name, fluid_type_name, typesofengineoil_name } =
              combinedLubeOilFilterData;
            ...
            const fluid_typeMatch =
              fluid_type_name.length === 0 ||
              fluid_type_name.includes(
                item.attributes.product_lubeoil_fluid_type.data.attributes
                  .fluid_type_name
              );
            const typesofengineoilMatch =
              typesofengineoil_name.length === 0 ||
              typesofengineoil_name.includes(
                item.attributes.product_typesofengineoil.data?.attributes
        ...
        if (storedTypes_json[0] === "tire") {
          const filteredData = tireData.filter((item: any) => {
            const { brand_name, diameter_name, width_name, ratio_name } =
              combinedTireFilterData;
            ...
            const diameterMatch =
              diameter_name.length === 0 ||
              diameter_name.includes(
                item.attributes.product_diameter.data.attributes.diameter_name
              );
            const widthMatch =
              width_name.length === 0 ||
              width_name.includes(
                item.attributes.product_width.data.attributes.width_name
              );
            const ratioMatch =
              ratio_name.length === 0 ||
              ratio_name.includes(
                item.attributes.product_ratio.data?.attributes.ratio_name
              );
  ...
  return (
    <React.Fragment>
      ...
      <Grid container justifyContent={"center"}>
            ...
            >
              <ThemeProvider theme={font}>
                <Typography sx={{ color: "black", fontSize: "16px" }}>
                  {`${t("Filter:showing")} ${Math.min(
                    (page - 1) * postPerPage + 1,
                    totalProducts
                  )}–${Math.min(
                    (page - 1) * postPerPage + productPerPage,
                    totalProducts
                  )} ${t("Filter:of")} ${totalProducts} ${t("Filter:results")}`}
                </Typography>
              </ThemeProvider>
            </Grid>
            <Grid
              item
              xs={12}
              sx={{ mt: 2, mb: 3, justifyContent: "center", display: "flex" }}
            >
              <Select
        ...
        <Grid item xs={isDesktop ? 9.5 : 11} sx={{}}>
          {isDesktop && (
            // ----------------------------------------------------- Desktop ------------------------------------------------------------------
            <Grid item xs={12}>
              <Grid container>
                              ...
                              >
                                ...
                                >
                                  <Image
                                    src={`${
                                      process.env
                                        .NEXT_PUBLIC_APP_STRAPI_BASE_URL
                                    }${

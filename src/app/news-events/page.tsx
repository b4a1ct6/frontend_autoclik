...
import { fecth_news_events, fecth_tip_trick, fetch_promotion } from '../store/slices/blog_contentSlice';
...
export default function news_events() {
    ...
    const Background = styled("div")(({ theme }) => ({
      ...
      const [news_events_Data, setNews_events_Data] = useState<any>([])

      useEffect(() =>{
        dispatch(fecth_news_events()).then((res:any) =>{
            setNews_events_Data(res.payload)
        })
      },[dispatch])
          ...
          <Grid container sx={{justifyContent:'center', display:'flex'}}>
                    <Grid item xs={isDesktop?10:11} sx={{height:'',mt:5}}>
                        ...
                        <Grid item xs={12} sx={{mt:isDesktop?5:0,pl:isDesktop?10:0,pr:isDesktop?10:0}}>
                            <Grid container sx={{}}>
                            {news_events_Data.map((row:any,index:any) =>(
